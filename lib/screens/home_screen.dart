...
import '../widgets/calendar_dialog.dart'; // новый импорт
  ...
  void _showCalendarDialog() {
    showDialog(
      ...
      builder: (ctx) => CalendarDialog(
        ...
        onDateSelected: (date) {
          // Здесь можно передать выбранную дату в PlanTab
          // Например, через callback или глобальное состояние
          // Для демонстрации показываем SnackBar
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Выбран день: ${date.toLocal().toString().split(' ')[0]}'),
            ),
          );
        },
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: GlassAppBar(
        ...
        onCalendarTap: _showCalendarDialog, // передаём колбэк
        config: config,
      ),
      body: TabBarView(
